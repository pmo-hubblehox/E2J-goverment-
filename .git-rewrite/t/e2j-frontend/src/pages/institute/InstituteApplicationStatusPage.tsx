...
import { Building2, ClipboardList, ArrowRight, CheckCircle, Clock, AlertCircle, XCircle } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';
import api from '../../services/api';

const QUICK_LINKS = [
  { icon: Building2,    title: 'Institute Profile',   description: 'View and manage your registered institute information.', to: '/institute/pending/profile', color: '#4338CA', bg: '#EEF2FF' },
  { icon: ClipboardList, title: 'Application Status', description: 'Track the current status of your onboarding application.',  to: '/institute/pending/status',  color: '#0F766E', bg: '#F0FDFA' },
];
...
const STATUS_CONFIG: Record<string, StatusInfo> = {
  PENDING: {
    ...
    titleColor: '#475569', msgColor: '#64748B', icon: Clock,
    title: 'Onboarding Incomplete',
    message: 'Complete your setup to submit your application for review.',
  },
  SUBMITTED: {
...
export default function InstituteApplicationStatusPage() {
  ...
  const { user } = useAuth();
  const firstName = user?.name?.split(' ')[0] ?? 'Institute';
  ...
  useEffect(() => {
    api.get('/institute/application/status')
      .then(res => {
        ...
        setAppStatus(status);
        if (status === 'APPROVED') navigate('/institute', { replace: true });
      })
      .catch(() => setAppStatus('PENDING'));
  }, []);

  const cfg: StatusInfo = STATUS_CONFIG[appStatus ?? 'PENDING'] ?? STATUS_CONFIG.PENDING;
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '900px' }}>
      ...
      {/* Welcome banner */}
      <div style={{ background: 'linear-gradient(135deg, #3F41D1 0%, #7C3AED 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', color: '#fff' }}>
        ...
        <p style={{ margin: 0, fontSize: '14px', opacity: 0.85, lineHeight: 1.6 }}>
          Thank you for registering with HubbleHox. Complete your onboarding to unlock full platform access.
        </p>
      </div>
