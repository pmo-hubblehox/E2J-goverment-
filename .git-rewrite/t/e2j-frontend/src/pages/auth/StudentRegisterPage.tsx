...
export default function StudentRegisterPage() {
  ...
  const onSubmit = async ({ email }: FormData) => {
    setServerError('');
    try {
      const res = await api.post('/auth/student/register/otp', { email });
      navigate('/register/otp', { state: { email, demoOtp: res.data?.data } });
    } catch (e: any) {
    ...
    <div style={card}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ position: 'relative' }}>
          <input type="email" autoComplete="email" placeholder="maulika.shah@gmail.com"
        ...
        <p style={{ textAlign: 'center', fontSize: '14px', color: '#666666', margin: 0 }}>
          ...
          <Link to="/login/student" style={{ color: '#3F41D1', fontWeight: 500, textDecoration: 'none', textTransform: 'capitalize' }}>Sign In</Link>
        </p>
      </form>
    </div>
  );
