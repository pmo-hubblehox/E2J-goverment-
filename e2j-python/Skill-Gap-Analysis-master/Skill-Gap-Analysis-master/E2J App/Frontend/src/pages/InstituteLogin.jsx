...
import { instituteLogin } from "../services/authService";
...
export default function InstituteLogin() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      const data = await instituteLogin({ email: email.trim(), password });
  ...
  return (
    <AuthLayout>
      ...
      <div className="small-link">
        Don’t have an account? <Link to="/register/institute">Register</Link>
      </div>
    </AuthLayout>
  );
}