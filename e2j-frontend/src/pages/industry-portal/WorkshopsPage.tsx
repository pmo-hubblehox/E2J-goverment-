...
interface Trainer { id: number; smeName: string; email: string; }
interface Workshop {
  ...
  status: string; rejectionReason: string; rating: number | null; trainerName: string;
  prerequisite: string | null; industrySmeId: number | null;
}
interface RosterRow {
...
function RosterModal({ workshop, onClose }: { workshop: Workshop; onClose: () => void }) {
  const [rows, setRows] = useState<RosterRow[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get(`/industry-portal/workshops/${workshop.id}/enrollments`)
...
function ReviewsModal({ workshop, onClose }: { workshop: Workshop; onClose: () => void }) {
  ...
  useEffect(() => {
    api.get(`/industry-portal/workshops/${workshop.id}/reviews`)
      .then(res => setRows(res.data?.data ?? []))
      .catch(() => setRows([]))
      .finally(() => setLoading(false));
  }, [workshop.id]);
...
export default function WorkshopsPage() {
  ...
  const [industrySmeId, setIndustrySmeId] = useState('');
  ...
  const load = () => {
    api.get('/industry-portal/workshops').then(res => setWorkshops(res.data?.data ?? [])).catch(() => setWorkshops([]));
    api.get('/industry-portal/sme').then(res => setTrainers(res.data?.data ?? [])).catch(() => setTrainers([]));
  };

  useEffect(() => { load(); }, []);

  const resetForm = () => {
    setTitle(''); setDescription(''); setTargetRoles([]); setIndustrySmeId(''); setMode('ONLINE');
  ...
  const openEdit = (w: Workshop) => {
    ...
    setIndustrySmeId(w.industrySmeId ? String(w.industrySmeId) : '');
  ...
  const handleAdd = async () => {
    ...
    const payload = {
      ...
      industrySmeId: industrySmeId ? Number(industrySmeId) : null,
    ...
    try {
      if (editId) await api.put(`/industry-portal/workshops/${editId}`, payload);
      else await api.post('/industry-portal/workshops', payload);
  ...
  return (
    <div style={{ padding: '32px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <Field label="Trainer (from SME Listing)">
                <select value={industrySmeId} onChange={e => setIndustrySmeId(e.target.value)} style={inputStyle}>
                  ...
                  {trainers.map(t => <option key={t.id} value={t.id} disabled={!t.email}>{t.smeName}{!t.email ? ' (no email on file)' : ''}</option>)}
                </select>
              </Field>
              <Field label="Mode">
      ...
      {rosterTarget && <RosterModal workshop={rosterTarget} onClose={() => setRosterTarget(null)} />}
