...
export default function IndustryRegisterPage() {
  ...
  const onSubmit = async ({ email }: FormData) => {
    setServerError('');
    try {
      const res = await api.post('/auth/industry/register/otp', { email });
      navigate('/register/otp', { state: { email, role: 'industry', demoOtp: res.data?.data } });
    } catch (e: any) {
    ...
    <div style={card}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ position: 'relative' }}>
          <input type="email" autoComplete="email" placeholder="partner@company.com"
            className="auth-field"
            style={{ ...fieldInput, borderColor: errors.email ? '#E6393E' : '#A3A3A3' }}
            {...register('email')} />
          <label style={fieldLabel}>
            Enter Company Email Id <span style={{ color: '#E6393E', marginLeft: '2px' }}>*</span>
          </label>
        ...
        <p style={{ textAlign: 'center', fontSize: '14px', color: '#666666', margin: 0 }}>
          ...
          <Link to="/login/industry" style={{ color: '#3F41D1', fontWeight: 500, textDecoration: 'none', textTransform: 'capitalize' }}>Sign In</Link>
        </p>
      </form>
    </div>
  );
